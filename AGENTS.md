...
- Global routes in `.codex/global-skills.json` are provisioned from immutable `tonglam/codex-workspace-config@7e92336ec04d38f7bb95620e304ce6ec6567c896:registry/workspace-assets.json` with its recorded SHA-256 content digest into the host Codex mount. Provision that source before invoking a route; prefer an authenticated local checkout by setting `CODEX_WORKSPACE_CONFIG_CHECKOUT` and run `python3 .codex/provision_global_skills.py --manifest .codex/global-skills.json --registry-source "$CODEX_WORKSPACE_CONFIG_CHECKOUT" --apply` when the host mount is absent. If an authenticated checkout is unavailable and network access is explicitly approved, use the same command with `--allow-network`; otherwise stop and report the missing dependency rather than silently substituting it or fetching implicitly.
