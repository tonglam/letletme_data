...
- Codex results can be stored in either of two GitHub API locations: findings may be formal pull-request reviews/threads (`pulls/<number>/reviews`), while a clean result may be an ordinary PR issue comment (`issues/<number>/comments`). Always inspect both locations; do not treat an empty `reviewDecision` or the absence of a formal approval as evidence that review is still pending.
- Accept a Codex clean gate only when the result explicitly says it found no major issues/no findings and includes `Reviewed commit: <sha>`. Compare that SHA (including an abbreviated but unambiguous prefix) with the full current `headRefOid`, and record the comment/review URL and timestamp. A generic acknowledgement, `mergeStateStatus: CLEAN`, green CI, review silence, or a review of an older commit is not sufficient.
- Once an exact-head clean result is found, stop polling and do not issue a timeout retry for that head—even if the result is a plain issue comment or arrives after a prior request appeared to time out. A timeout retry is allowed only after checking both API locations and confirming that no matching result or active request exists.
