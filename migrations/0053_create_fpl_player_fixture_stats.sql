-- Per-fixture FPL evidence retained before event-level aggregation.
CREATE TABLE IF NOT EXISTS public.fpl_player_fixture_stats (
  ...
  CONSTRAINT fpl_player_fixture_stats_nonnegative_check CHECK (
    minutes >= 0 AND (starts IS NULL OR starts BETWEEN 0 AND 1)
    AND goals >= 0 AND assists >= 0
    AND own_goals >= 0 AND yellow_cards >= 0 AND red_cards >= 0
  )
);
ALTER TABLE public.fpl_player_fixture_stats ALTER COLUMN starts DROP NOT NULL;
