...
-- Establish a direct rebuild fence for terminal events. This deliberately
-- reads only the event identity and active publication identity; it does not
-- inspect existing snapshot children or reconstruct any historical scope.
-- Every existing FINAL is rebuilt once by the current capture path, which
-- writes the current payload and verifies both generations in the same
-- transaction. ON CONFLICT DO NOTHING preserves a newer trigger-owned state
-- when a scope mutation commits while this migration is running.
...
SELECT event.season_id,
       ...
       CASE WHEN publication.revision IS NOT NULL THEN 1 ELSE 0 END,
       0,
       CASE WHEN publication.revision IS NOT NULL THEN clock_timestamp() END,
       CASE WHEN publication.revision IS NOT NULL THEN clock_timestamp() END,
       NULL,
       NULL,
       clock_timestamp()
