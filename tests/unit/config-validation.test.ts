...
describe('production environment preflight', () => {
  ...
  test('restores stopped services when a pre-migration deployment gate rejects', () => {
    ...
    const databaseQuiescenceCommand =
      'compose run --rm -T --no-interactive migration bun scripts/assert-queue-quiescence.ts --database-only --scoped';
    const redisQuiescenceCommand =
      'compose run --rm -T --no-interactive api bun scripts/assert-queue-quiescence.ts --redis-only --scoped';
    ...
    expect(deployScript).toMatch(
      /if ! compose run --rm -T --no-interactive migration bun scripts\/assert-queue-quiescence\.ts --database-only --scoped; then[\s\S]*?restore_stopped_services[\s\S]*?exit 1[\s\S]*?fi/,
    );
    ...
    expect(deployScript).toMatch(
      /if ! compose run --rm -T --no-interactive api bun scripts\/assert-queue-quiescence\.ts --redis-only --scoped; then[\s\S]*?restore_stopped_services[\s\S]*?exit 1[\s\S]*?fi/,
    );
