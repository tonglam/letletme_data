  ...
  describe('Bootstrap API Integration', () => {
    describe('Successful Data Fetching', () => {
      ...
      describe('Phases Data', () => {
        it(
          'should successfully fetch and validate phases data',
          async () => {
            const data = await bootstrapAdapter.getBootstrapData();

            // Verify phases data structure
            expect(Array.isArray(data.phases)).toBe(true);
            expect(data.phases.length).toBeGreaterThan(0);

            // Verify required fields in first phase
            const phase = data.phases[0];
            expect(phase).toMatchObject({
              id: expect.any(Number),
              name: expect.any(String),
              start_event: expect.any(Number),
              stop_event: expect.any(Number),
            });

            // Verify phase range is valid
            expect(phase.start_event).toBeLessThanOrEqual(phase.stop_event);
          },
          TEST_TIMEOUT,
        );
      });
      ...
      describe('Teams Data', () => {
        it(
          'should successfully fetch and validate teams data',
          async () => {
            const data = await bootstrapAdapter.getBootstrapData();
            ...
            // Verify required fields in first team
            ...
            expect(team).toMatchObject({
              ...
              strength_overall_home: expect.any(Number),
              strength_overall_away: expect.any(Number),
              strength_attack_home: expect.any(Number),
              strength_attack_away: expect.any(Number),
              strength_defence_home: expect.any(Number),
              strength_defence_away: expect.any(Number),
            });

            // Verify team strength values are within expected range (1-5)
      ...
      describe('Elements Data', () => {
        it(
          'should successfully fetch and validate elements data',
          async () => {
            const data = await bootstrapAdapter.getBootstrapData();

            // Verify elements data structure
            expect(Array.isArray(data.elements)).toBe(true);
            expect(data.elements.length).toBeGreaterThan(0);

            // Verify required fields in first element (player)
            const element = data.elements[0];
            expect(element).toMatchObject({
              id: expect.any(Number),
              web_name: expect.any(String),
              team: expect.any(Number),
              element_type: expect.any(Number),
              now_cost: expect.any(Number),
              total_points: expect.any(Number),
              minutes: expect.any(Number),
              goals_scored: expect.any(Number),
              assists: expect.any(Number),
              clean_sheets: expect.any(Number),
              goals_conceded: expect.any(Number),
              own_goals: expect.any(Number),
              penalties_saved: expect.any(Number),
              penalties_missed: expect.any(Number),
              yellow_cards: expect.any(Number),
              red_cards: expect.any(Number),
              ...
              ict_index: expect.any(String),
            });

            // Verify numeric values are non-negative
            expect(element.now_cost).toBeGreaterThanOrEqual(0);
            expect(element.total_points).toBeGreaterThanOrEqual(0);
            expect(element.minutes).toBeGreaterThanOrEqual(0);
          },
          TEST_TIMEOUT,
        );
      });
