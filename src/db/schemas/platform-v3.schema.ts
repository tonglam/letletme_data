...
export const laneInUnderstat = understat.enum('lane', ['team', 'player']);
export const seasonStateInUnderstat = understat.enum('season_state', [
  'planned',
  'active',
  'complete',
]);
export const syncItemStatusInUnderstat = understat.enum('sync_item_status', [
  'pending',
  'running',
  'failed',
  'completed',
...
export const syncModeInUnderstat = understat.enum('sync_mode', [
  'incremental',
  'full',
  'reconcile',
]);
export const syncRunStatusInUnderstat = understat.enum('sync_run_status', [
  'pending',
  'running',
  'failed',
  'completed',
...
export const syncTriggerInUnderstat = understat.enum('sync_trigger', ['cron', 'manual', 'api']);
...
export const matchesInUnderstat = understat.table(
  'matches',
  {
    ...
    homeXg: numeric('home_xg'),
    awayXg: numeric('away_xg'),
    forecastHomeWin: numeric('forecast_home_win'),
    forecastDraw: numeric('forecast_draw'),
    forecastAwayWin: numeric('forecast_away_win'),
    sourceHash: text('source_hash').notNull(),
...
export const playerMatchStatsInUnderstat = understat.table(
  'player_match_stats',
  {
    ...
    xg: numeric().notNull(),
    xa: numeric().notNull(),
    xgChain: numeric('xg_chain').notNull(),
    xgBuildup: numeric('xg_buildup').notNull(),
...
export const teamStatSplitsInUnderstat = understat.table(
  'team_stat_splits',
  {
    ...
    xgFor: numeric('xg_for').notNull(),
    shotsAgainst: integer('shots_against').notNull(),
    goalsAgainst: integer('goals_against').notNull(),
    xgAgainst: numeric('xg_against').notNull(),
    sourceHash: text('source_hash').notNull(),
    createdAt: timestamp('created_at', { withTimezone: true, mode: 'date' }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true, mode: 'date' }).defaultNow().notNull(),
  },
...
export const playerTeamSeasonsInUnderstat = understat.table(
  'player_team_seasons',
  {
    ...
    redCards: integer('red_cards').notNull(),
    xg: numeric().notNull(),
    nonPenaltyXg: numeric('non_penalty_xg').notNull(),
    xa: numeric().notNull(),
    xgChain: numeric('xg_chain').notNull(),
    xgBuildup: numeric('xg_buildup').notNull(),
    position: text().notNull(),
  ...
  (table) => [
    ...
    index('understat_player_team_team_season_fk_idx').using(
      'btree',
      table.seasonCode.asc().nullsLast(),
      table.teamId.asc().nullsLast(),
    ),
    ...
    foreignKey({
      columns: [table.seasonCode, table.teamId],
      foreignColumns: [teamSeasonsInUnderstat.seasonCode, teamSeasonsInUnderstat.teamId],
      name: 'understat_player_team_team_season_fk',
    }),
    primaryKey({
...
export const teamMatchStatsInUnderstat = understat.table(
  'team_match_stats',
  {
    ...
    xg: numeric().notNull(),
    xga: numeric().notNull(),
    npxg: numeric().notNull(),
    npxga: numeric().notNull(),
    npxgd: numeric().notNull(),
...
export const playerValueChangesInReporting = reporting
  ...
  .with({ securityInvoker: true })
  .as(
    sql`WITH ordered_snapshots AS ( SELECT snapshot.season_id, snapshot.snapshot_date, snapshot.element_id, snapshot.element_type, snapshot.price, snapshot.snapshot_source, snapshot.source_value_id, snapshot.source_event_id, lag(snapshot.price) OVER (PARTITION BY snapshot.season_id, snapshot.element_id ORDER BY snapshot.snapshot_date) AS previous_price, row_number() OVER (PARTITION BY snapshot.season_id, snapshot.element_id ORDER BY snapshot.snapshot_date) AS snapshot_number FROM fpl.player_market_snapshots snapshot ), changed_snapshots AS ( SELECT ordered.season_id, ordered.snapshot_date, ordered.element_id, ordered.element_type, ordered.price, ordered.snapshot_source, ordered.source_value_id, ordered.source_event_id, ordered.previous_price, ordered.snapshot_number FROM ordered_snapshots ordered WHERE ordered.snapshot_number = 1 OR ordered.price IS DISTINCT FROM ordered.previous_price ) SELECT changed.season_id, season.season_code, changed.snapshot_date, changed.element_id, changed.element_type, COALESCE(changed.source_event_id, event.event_id) AS event_id, changed.price AS value, CASE WHEN changed.snapshot_number = 1 THEN 0 ELSE changed.previous_price END AS last_value, CASE WHEN changed.snapshot_number = 1 THEN 'start'::text WHEN changed.price > changed.previous_price THEN 'rise'::text ELSE 'fall'::text END AS change_type, CASE WHEN changed.snapshot_number = 1 THEN changed.price ELSE changed.price - changed.previous_price END AS value_change, changed.snapshot_source, changed.source_value_id FROM changed_snapshots changed JOIN fpl.seasons season ON season.season_id = changed.season_id LEFT JOIN fpl.events event ON event.season_id = changed.season_id AND event.deadline_time::date = changed.snapshot_date`,
  );
