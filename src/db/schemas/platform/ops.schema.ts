...
export const schedulerObligationsInOps = ops.table(
  ...
  (table) => [
    ...
    index('scheduler_obligations_nonterminal_job_idx')
      .on(table.jobName)
      .where(sql`status NOT IN ('succeeded', 'skipped', 'irrecoverable')`),
...
export const queueHealthWindowsInOps = ops.table(
  'queue_health_windows',
  {
    ...
    updatedAt: timestamp('updated_at', { withTimezone: true, mode: 'date' })
      .default(sql`clock_timestamp()`)
      .notNull(),
    consumerPaused: boolean('consumer_paused'),
    pausedCount: integer('paused_count'),
    pauseOwnerState: text('pause_owner_state'),
  },
  (table) => [
    primaryKey({
    ...
    check(
      ...
      sql`waiting >= 0 AND active >= 0 AND delayed >= 0 AND prioritized >= 0 AND waiting_children >= 0 AND (paused_count IS NULL OR paused_count >= 0) AND failed >= 0 AND completed >= 0 AND runnable >= 0 AND arrivals >= 0 AND completions >= 0 AND failures >= 0 AND stalled >= 0`,
    ),
    check(
      'queue_health_windows_pause_owner_check',
      sql`pause_owner_state IS NULL OR pause_owner_state = ANY (ARRAY['NONE','DEPLOYMENT','ACQUIRING','OPERATOR','RELEASING'])`,
    ),
    check(
