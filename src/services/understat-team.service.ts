...
export async function discoverUnderstatTeams(
  job: UnderstatTeamJobData,
  ...
  const mutation = {
    ...
    jobName: 'understat-team-discover',
    scopes: understatMutationScopes('team', 'understat-team-discover', job.season, job.teamId),
  };
  const claim = await withMutationScopes(mutation, async () => {
    const priorItems = await understatSyncRepository.findUnsettledItems(job.season, 'team');
    ...
    return { priorItems, sameRunTeamIds, attempt };
  });
  ...
  await withMutationScopes(mutation, async () => {
    if (
      ...
      return;
    const { priorItems, sameRunTeamIds } = claim;
    ...
    handoffs.push(await enqueueTeamDetailJobs(job, targetIds, teams));
    handoffs.push(await finalizeWhenReady(job, ready));
  });
  for (const handoff of handoffs) await handoff();
}

export async function syncUnderstatTeamDetail(
  job: UnderstatTeamJobData,
  onClaim?: (attempt: number) => void,
): Promise<void> {
  const handoffs: Array<() => Promise<void>> = [];
  ...
  const mutation = {
    queueName: 'understat-team-sync',
    jobName: 'understat-team-detail',
    scopes: understatMutationScopes('team', 'understat-team-detail', job.season, job.teamId),
  };
...
export async function finalizeUnderstatTeamRun(job: UnderstatTeamJobData): Promise<void> {
  const mutation = {
    queueName: 'understat-team-sync',
    jobName: 'understat-team-finalize',
    scopes: understatMutationScopes('team', 'understat-team-finalize', job.season, job.teamId),
  };
  await withMutationScopes(mutation, async () => {
