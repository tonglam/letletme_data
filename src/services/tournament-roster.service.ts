...
async function assertPreGameweekBoundary(): Promise<void> {
...
async function reconcileTournamentRosterUnlocked(
  ...
  const tournament = await tournamentRosterRepository.findById(tournamentId);
  if (!tournament) {
  ...
  try {
    ...
    const existingIds = await tournamentRosterRepository.findEntryIds(tournamentId);
...
export async function reconcileOfficialTournamentRosters(): Promise<{
  total: number;
  changed: number;
  skipped: number;
  errors: number;
  ...
  const tournaments = await tournamentRosterRepository.findActiveOfficialSync();
  let changed = 0;
  let errors = 0;
  await mapWithConcurrency(tournaments, 2, async (tournament) => {
    try {
      const result = await reconcileTournamentRoster(tournament.id);
