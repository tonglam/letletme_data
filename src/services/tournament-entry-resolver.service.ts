...
export interface TournamentEntryResolverDependencies {
  findStoredEntryIds: (tournamentId: number) => Promise<number[]>;
...
const defaultDependencies: TournamentEntryResolverDependencies = {
  findStoredEntryIds: (tournamentId) =>
    tournamentEntryRepository.findEntryIdsByTournamentId(tournamentId),
...
export async function resolveTournamentEntryIds(
  ...
  const storedEntries = uniqueNumbers(await dependencies.findStoredEntryIds(tournament.id));
