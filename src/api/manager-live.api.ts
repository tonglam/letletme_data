...
export const createManagerLiveAPI = (
  resolve: typeof resolveManagerLiveScores = resolveManagerLiveScores,
) =>
  new Elysia({ prefix: '/internal/manager-live' }).post(
    '/resolve',
    async ({ body }) => {
      const data = await resolve(body);
...
export const managerLiveAPI = createManagerLiveAPI();
