  ...
  try {
    ...
    if (exists === 1) {
      ...
      if (hashSize > 0) {
        ...
        console.log(`[LiveBonus] Sample team ${firstTeamId}:`, JSON.stringify(firstTeamData, null, 2));
      }
    } else {
    ...
    console.log(`[LiveBonus] Cache get() result: ${cached ? `${Object.keys(cached).length} teams` : 'null'}`);
    ...
    if (eventLivesWithTeam.length > 0) {
      ...
      console.log(`[LiveBonus] Event lives: ${withMinutes.length} with minutes > 0, ${withBps.length} with BPS > 0, ${withBonus.length} with bonus > 0`);
