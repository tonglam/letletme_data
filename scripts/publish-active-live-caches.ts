...
import type { LiveBonusByTeam } from '../src/domain/live-bonus';
...
function assertArguments(args: readonly string[]): boolean {
  ...
  if (unknown) throw new Error(`Unknown active-live publication argument: ${unknown}`);
  ...
  for (const candidate of rows) {
    ...
    if (execute) {
      ...
      if (!published.published) {
        throw new Error(`Canonical live publication lost its database fence: ${row.publicationId}`);
      }
...
if (import.meta.main) {
  main()
    .catch((error) => {
      console.error('[publish-active-live-caches] failed', error);
      process.exitCode = 1;
    })
    .finally(async () => {
