...
- 迁移前的旧 all-X policy 曾对 83 个 X account Endpoint 逐一执行真实 `x_user_search`。旧 manifest
  中 Aston Villa、Brighton、Coventry 的官方 handle 分别修正为 `AstonVilla`、`BHAFC`、
  `CoventryCityFC`；修正后该历史批次 83/83 均以 exact case-insensitive handle 绑定唯一数字 user
  ID。隔离执行合同复跑为 82/83 首次通过，唯一一次 transient miss 仅重试 Aston Villa 后达到
  83/83；该轮共 84 次有 trace 调用，约 USD 0.8327。0044 policy 后，只有 22 个官方 X account
  继续需要 numeric identity，其余 manifest 账号按 handle 扫描。
...
- [x] 22/22 官方账号 exact identity resolution；44 个唯一 partition + 5 个 keyword follow-up
  完整 sweep 为 49/49 trace、0 failure、0 rejected。其余 manifest X 账号按 `HANDLE_ONLY` 扫描，
  不再把 numeric identity resolution 作为 gate。
...
- [x] X partition sweep 已在隔离 PostgreSQL 15 路径完成；semantic 日期精度、cap 和不分页语义已
  用真实结果验证。旧 all-X identity sweep 只作为历史证据保留；当前 identity gate 只覆盖 22 个
  官方账号，handle-only 与 discovered-only 账号不进入 identity scheduler。
...
附近，第二轮完整 shadow 完成于 `2026-08-21T19:35Z` 附近；旧版 83 个 identity 与 49-call 完整
X sweep 于 2026-08-22 完成。当前 0044 policy 的目标是 22 个官方 identity，其余 X 账号按 handle
扫描。
