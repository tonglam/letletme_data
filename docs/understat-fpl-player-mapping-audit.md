...
> Historical evidence snapshot generated before the platform consolidation. Runtime names are
...
- 规则 ID：`understat-fpl-player-name`
...
- 1617–2526 的审计当时使用按赛季归档的 FPL 数据；平台统一后这些事实位于显式
  `season_id` 的 `fpl.*` 表。1415、1516 使用本地保存的官方 Pulselive ranked stats
  identity/metrics 与 FPL historical stats；这些来源只作为 bridge evidence，不创建 history 分区。
