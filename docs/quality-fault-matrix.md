...
| --- | --- | --- |
| Unit environment or provider reaches the network | `tests/unit/pr7-fault-matrix.test.ts`, `tests/unit/preload.ts` | Loopback synthetic endpoints and a throwing `fetch`; no remote credentials are retained. |
